...
- [ ] Yes — `bash scripts/run-eval.sh` 결과를 아래에 첨부 (schema validation + semantic coverage + LLM-as-judge + baseline diff 가 머지 게이트. ADR-006 / CLAUDE.md 참조)
...
- LLM-judge 점수: <예: avg 4.3 / 5, 샘플 수 N>
- Baseline diff: <회귀 없음 | 회귀 항목 요약 + 판단 근거>

> LLM-as-judge 는 API 비용 때문에 opt-in: `bash scripts/run-eval.sh --llm-judge`. 회귀 차단을 엄격히 원하면 `--fail-on-regression`.
